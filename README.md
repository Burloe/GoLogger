...
A potential pitfall to consider when logging large/growing data is how Godot's `FileAccess` handles file writing. The `FileAccess.WRITE` mode truncates(deletes) the file's content, meaning we can't simply write a new entry to the file. The plugin first stores each entry in an array. The entries are then re-entered sequentially before appending a new entry. This process can cause performance issues when log files have excessive entries, potentially leading to stuttering and/or slowdowns. It is therefore vital to have limiters in place to prevent log files from becoming too large. GoLogger has two settings that you can change in the editor dock under the names `Limit Method` and `Limit Action`. The method refers to the condition(or method) that triggers the limit, while the action refers to the action taken.<br><br>
...
* `Stop session` will of course stop the session once the cap is hit. Requires you to start the session either manually or through code to log again.
* `Stop & start session` restarts a sessios, creating a new log file.
